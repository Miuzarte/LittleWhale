// Ported from the miuix example app's `navigation/CrossActivityTransition.kt`, via SFA's copy of it.
        ...
        when {
            ...
            g != null -> (
                scope.relativeDepth.coerceIn(0f, 1f) /
                    (1f - g.progress).coerceAtLeast(MIN_RELEASE_SPAN)
                ).coerceIn(0f, 1f)
...
/**
 * 平台那种跨 activity 的手感 (AOSP WM Shell), 转写成一条分方向的过渡
 *
 * 这是设置页「过渡风格」里的 AOSP: 页面缩小一层、从边上让开, 而不是 Miuix 那种整页平移
 */
